    ...
    export default {
        ...
        methods: {
            ...
            loadImageCode: function () {
                ...
                $('#image-code').attr('src', process.env.VUE_APP_SERVER + '/system/admin/kaptcha/imageCode/' + _this.imageCodeToken);
            },
            login(){
                let _this=this;
                // 保存校验
                if (1 != 1
                    || !Validator.require(_this.member.mobile, "手机名")
                    || !Validator.require(_this.member.password, "密码")
                    || !Validator.require(_this.member.imageCode, "验证码")
                ) {
                    return;
                }
                ...
                _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/member/login',_this.member)
                    .then((response)=>{
                        ...
                        if (response.data.boo) {
                            ...
                            Tool.setLoginMember(response.data.content);
                            ...
                            // 登录成功 TODO
                            // 登录成功
                            _this.$parent.setLoginMember(loginUser);
                            $("#login-modal").modal("hide");

                        }else {
                            prompt.error(response.data.message);
                            _this.member.password = "";
                            _this.member.imageCode = "";
                            _this.loadImageCode();
                        }
                    })
            ...
            sendSmsForRegister() {
                ...
                _this.$ajax.get(process.env.VUE_APP_SERVER + '/business/web/member/isMobileExist/' + _this.memberRegister.mobile).then((res)=>{
                    let response = res.data;
                    if (response.boo) {
                        prompt.warning("手机号已被注册");
                    } else {
                        // 调服务端发送短信接口
            ...
            sendSmsCode(sms,btnId) {
                ...
                _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/sms/send', sms).then((res)=> {
                    let response = res.data;
                    if (response.boo) {
                        prompt.success("短信已发送")
                        // 开始倒计时
                        _this.countdown = 60;
                        _this.setTime(btnId);
                    } else {
                        prompt.warning(response.message);
                    }
                })
            },
            /**
            ...
            sendSmsForForget() {
                ...
                _this.$ajax.get(process.env.VUE_APP_SERVER + '/business/web/member/isMobileExist/' + _this.memberForget.mobile).then((res)=>{
                    let response = res.data;
                    if (response.boo) {
            ...
            resetPassword() {
                ...
                // 调服务端密码重置接口
                _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/member/resetPassword', _this.memberForget).then((res)=>{
                    let response = res.data;
                    if (response.boo) {
                        prompt.success("密码重置成功");
                        _this.toLoginDiv();
                    } else {
                        prompt.warning(response.message);
                    }

...
public class RoleResourceController {
        //输出日志的服务名称
     public static final String BUSINESS_NAME="SYSTEM_NAME服务之(RoleResourceController)";
    @Autowired
    ...
    public ResponseVo getList(@RequestBody PageDto pageDto){
        roleResourceService.getList(pageDto);
    ...
    /**
     * 添加和修改的共用类
     * @param roleResourceDto  添加与修改的dto实体类
     * @return   返回共用的json数据vo模版格式
     */
    @ApiOperation(value = "添加和修改的共用类")
    @PostMapping("/saveAndUpdate")
    public ResponseVo saveAndUpdate(@RequestBody RoleResourceDto roleResourceDto){
        /**
         * 进行数据的长度和非空验证
         */

            ValidatorUtils.require(roleResourceDto.getRoleId(), "角色");
            ValidatorUtils.require(roleResourceDto.getResourceId(), "资源");

        roleResourceService.saveAndUpdate(roleResourceDto);
        ResponseVo responseVo = new ResponseVo();
        responseVo.setCode("200").setMessage("操作成功").setContent(roleResourceDto);
        return  responseVo;
    }

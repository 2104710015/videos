...
public class ChapterController {
        //输出日志的服务名称
     public static final String BUSINESS_NAME="CUSTOMER_NAME服务之(ChapterController)";
    @Autowired
    ...
    /**
     * 添加和修改的共用类
     * @param chapterDto  添加与修改的dto实体类
     * @return   返回共用的json数据vo模版格式
     */
    @ApiOperation(value = "添加和修改的共用类")
    @PostMapping("/saveAndUpdate")
    public ResponseVo saveAndUpdate(@RequestBody ChapterDto chapterDto){
        /**
         * 进行数据的长度和非空验证
         */

            ValidatorUtils.length(chapterDto.getName(), "名称", 1, 50);

        chapterService.saveAndUpdate(chapterDto);
        ResponseVo responseVo = new ResponseVo();
        responseVo.setCode("200").setMessage("操作成功").setContent(chapterDto);
        return  responseVo;
    }
